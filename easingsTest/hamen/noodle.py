...
def spawnFakeNotesWithTrackAt(startTime, endTime, disableGravity=False, timeOffset=0, track='', disableDebris=False, uninteractable=False):
    for index in range(len(exData['colorNotes'])):
        if (startTime <= exData['colorNotes'][index]['b']) and (endTime >= exData['colorNotes'][index]['b']):
...
def childrenTracks(nTime, trackName, childrens, worldPositionStays=False):
    exData['customData']['customEvents'].append(dict(b=nTime, t='AssignTrackParent', d={'childrenTracks':childrens, 'parentTrack':trackName, 'worldPositionStays':worldPositionStays}))
...
def animateTrack(nTime, trackName, duration, easings='easeLinear', offsetPos=None, worldRotation=None, localRotation=None, scale=None, dissolve=None, dissolveArrow=None, interactable=None, time=None, pos=None, localPos=None):
    dat = {}
    # add essential stuff
    dat['track'] = trackName
    dat['duration'] = duration
