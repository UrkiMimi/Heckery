...
# fixes env errors
def sanityCheck():
    for index in range(len(exData['customData']['environment'])):
        if 'duplicate' in exData['customData']['environment'][index]:
            if exData['customData']['environment'][index]['duplicate'] == 0:
                exData['customData']['environment'][index].pop('duplicate')