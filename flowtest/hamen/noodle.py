...
# converts 360 map rotation to noodle rotation
def getCurrentDegreeAtTime(time):
    """This is a simple script that returns an approximate rotation based on BSV3's rotationEvents.
    This requires you to have your base map set to 360 or 90 degree with these populated.

    Args:
        time (float): Time (in beats)

    Returns:
        degrees (int): Approximate degrees
    """
    rot = 0
    for index in range(len(exData['rotationEvents'])):
        # return at given time
        if (exData['rotationEvents'][index]['b'] > time):
            return rot
        else:
            rot += exData['rotationEvents'][index]['r']
...
def dissolveBoth(nTime, trackName, duration, oldx,x, easing = 'easeLinear'):
    exData['customData']['customEvents'].append(dict(b=nTime, t='AnimateTrack', d={'duration':duration, 'repeat':0}))
    exData['customData']['customEvents'][len(exData['customData']['customEvents']) - 1]['d']['track'] = trackName
    exData['customData']['customEvents'][len(exData['customData']['customEvents']) - 1]['d']['dissolve'] = [
        [oldx,0],
...
# ripped from mawntee part 3
def circleThing(startTime, endTime, offset):
    for index in range(len(exData['colorNotes'])):
        if (exData['colorNotes'][index]['b'] >= startTime) and (exData['colorNotes'][index]['b'] <= endTime):
            deg = 0
            randdeg = rand.randint(-20,20)
            x = rand.randint(-5,5)
            y = rand.randint(0,5)
            y2 = rand.randint(0,5)
            ...
            exData['colorNotes'][index]['customData']['animation'] = {}
            exData['colorNotes'][index]['customData']['noteJumpMovementSpeed'] = 0.0001
            exData['colorNotes'][index]['customData']['noteJumpStartBeatOffset'] = offset
            exData['colorNotes'][index]['customData']['disableNoteLook'] = True
            exData['colorNotes'][index]['customData']['disableNoteGravity'] = True
            ...
            exData['colorNotes'][index]['customData']['animation']['offsetPosition'] = [
                [-x*2,y2,100,0],
                [x,y,20,0.2,'easeOutCirc'],
                [0,0,20,0.3,'easeInOutExpo'],
                [0,0,0,0.5,'easeInQuad'],
                [0,0,-20,0.7,'easeOutQuad']
            ]

            exData['colorNotes'][index]['customData']['animation']['offsetWorldRotation'] = [
                [0,randdeg+deg,0,0.2],
                [0,deg,0,0.3,'easeInOutExpo'],
            ]
